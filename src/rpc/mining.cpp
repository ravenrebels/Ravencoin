...
UniValue generateBlocks(std::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript)
{
    ...
    while (nHeight < nHeightEnd) {
        ...
        uint256 mix_hash;
        while (nMaxTries > 0 && pblock->nNonce < nInnerLoopCount && !CheckProofOfWork(pblock->GetHashFull(mix_hash), pblock->nBits, GetParams().GetConsensus())) {
            if (pblock->nTime < nKAWPOWActivationTime) {
                ++pblock->nNonce;
            } else {
                ++pblock->nNonce64;
            }
            static int64_t nLogTimeStart = 0;
            static uint64_t nLastHashes = 0;
            if (nLogTimeStart == 0) nLogTimeStart = GetTime();

            if (GetTime() - nLogTimeStart >= 5) {
                uint64_t nHashesNow = (pblock->nNonce64 > 0 ? pblock->nNonce64 : pblock->nNonce);
                uint64_t nDiff = nHashesNow - nLastHashes;
                if (nLastHashes != 0) { // Skip first valid calc to allow diff
                    LogPrintf("generatetoaddress: %llu hashes total, %llu H/s\n", nHashesNow, nDiff / 5);
                }
                nLogTimeStart = GetTime();
                nLastHashes = nHashesNow;
            }
            --nMaxTries;
        }
        if (nMaxTries == 0) {
...
UniValue getgenerate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            std::string(RAVEN_CONF_FILENAME) + " setting gen)\n"
